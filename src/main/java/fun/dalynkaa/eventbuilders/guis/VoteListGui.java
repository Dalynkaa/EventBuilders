...
public class VoteListGui {
    PaginatedGui gui;
    public VoteListGui(VoteFilter filter, Game game, PlotPlayer plotPlayer){
        gui = Gui.paginated()
        ...
        Bukkit.getLogger().info(String.valueOf(System.currentTimeMillis() - start) + " -1");
        plotListHashMap = PlotVote.getGameVotes(game);
        Bukkit.getLogger().info(String.valueOf(System.currentTimeMillis() - start) + " -2");
        ...
        for (Plot plot: plots){
            if (plotListHashMap.containsKey(plot.getPlotId())) {
                ...
                Integer summ = 0;
                for (PlotVote plotVote : votes) {summ += plotVote.getType();}
                forSort.add(plot.setPointSum(summ));
            }
        }
        forSort.sort(Comparator.comparingDouble(Plot::getPointSum).reversed());
        Bukkit.getLogger().info(String.valueOf(System.currentTimeMillis() - start) + " -3");
